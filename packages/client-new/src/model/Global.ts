...
Promise.all([img0, img1, img2]).then((list) => {
    Global.parts.update(list.map(img => new GunPart(img)))
})

Global.preview.scale.attach(R.effect((v) => console.log('work-screen scale change', v)))
Global.parts.attach(R.effect((v) => console.log('gun parts change', v)))